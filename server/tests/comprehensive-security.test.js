...
process.env.CAFE_LATITUDE = '0';
process.env.CAFE_LONGITUDE = '0';
process.env.CAFE_ALLOWED_RADIUS_METERS = '50';
...
test('real security integration suite', async (t) => {
  ...
  try {
    ...
    await t.test('server-side pricing ignores malicious client totals', async () => {
      const session = await json('/api/session', { tableNumber: 1, latitude: 0, longitude: 0, accuracy: 1 });
      const result = await json('/api/orders', {
        tableNumber: 1,
